...
export const SelectItem = React.forwardRef(function SelectItem(props, ref) {
    ...
    return (
        <ChakraSelect.Item key={item.value} item={item} color="fg" {...rest} ref={ref}>
            {children}
        ...
        <ChakraSelect.ValueText {...rest} ref={ref}>
            <ChakraSelect.Context>
                {(select) => {
                    ...
                    if (items.length === 0 || items[0] === null || items[0] === undefined || items[0] === "")
                        return <Text color="fg.muted">{props.placeholder}</Text>; // props.placeholder;
